...
public entry fun generate_number(random_number: u64) -> u64 {
  let mean = 5;
  let standard_deviation = 2;
  let number = (random_number as f64 * standard_deviation) + mean;
...
*/
 ...
 // Modify duel_player1 function to include validation and game rules
public entry fun duel_player1(player1: &signer, player2: address, wager: u64, card: &mut CARD) {
    assert!(signer::address_of(player1) != player2, ERR_UNAUTHORIZED);
    assert!(coin::balance<DuelToken>(signer::address_of(player1)) >= wager, ERR_INSUFFICIENT_FUNDS);
    ...
    // Determine the outcome of the duel and apply game rules
    let player1_net_power = card.power * card.multiplier;
    ...
    if (random_winner <= player1_net_power) {
        card.power = card.power + player1_net_power;
        coin::deposit<DuelToken>(signer::address_of(player1), wager * 2); // Player 1 wins
    } else {
        card.power = card.power + player2_net_power;
        coin::deposit<DuelToken>(player2, wager * 2); // Player 2 wins
    }
}

// Modify duel_player2 function similarly
public entry fun duel_player2(player2: &signer, player1: address, wager: u64, card: &mut CARD, duel: Duel) {
    assert!(signer::address_of(player2) != player1, ERR_UNAUTHORIZED);
    ...
    move_to(player2, Duel {
        player: signer::address_of(player2),
        opponent: player1,
        wager: coin::withdraw<DuelToken>(player2, wager),
        player_card: card,
    });
    ...
    if (random_winner <= player1_net_power) {
        ...
        coin::deposit<DuelToken>(player1, wager * 2); // Player 1 wins
    } else {
        card.power = card.power + duel.player_card.power;
        coin::deposit<DuelToken>(signer::address_of(player2), wager * 2); // Player 2 wins
    }
}
